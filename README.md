...
- 2023/04/01 更新：添加新的**绕过**教程 见[适用于NewBing的咒语](README.md#适用于NewBing的咒语)，添加[第三方插件](README.md#第三方插件)
...
- 这个工具能够避免服务器发出的话被撤回（无法解除服务器的审查导致的对话中断），没有了单次对话的限制（每日限制仍然存在），内置了绕过模板（base64绕过/ROT13绕过）
- **⚠警告⚠**使用此插件有一定的风险导致封号（建议不使用绕过模板仅仅使用其连续对话功能）
- 工具链接 https://greasyfork.org/en/scripts/461220-powerful-bing-chat-ai
