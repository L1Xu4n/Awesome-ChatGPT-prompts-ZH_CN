# -ChatGPT- 2022/12/20更新:添加了更稳定触发~涩涩~的教程
如何将ChatGPT调教成一只猫娘以及关于ChatGPT的帮助，涩涩的教程在更多玩法

# 🌟保存对话小工具🌟
- 在开始之前我强烈推荐你先准备好这个工具，它可以让你继续之前的对话（当然没有保存的不行），允许在高负载时登录，并且去除文本审查屏蔽
...
- 发送涩涩的设定模板
## 其他玩法有待开发中，如果你有好的想法可以提交 Issue 和 Pull request，如果已过期请记得Close

# 帮助
这里列出了一些你可能遇到问题的解决方案
- > 与猫娘对话的时候，猫娘总是发一些被划掉的话？

    由于ChatGPT 聊天页面使用了 Markdown 进行格式化，如果猫娘在发送的某段话的前后出现了两个波浪号，会被处理为删除线，可能表现为这样：喵呜~~最喜欢主人了喵~~
- > 有些话发到一半不发了？
...
- > 为什么猫娘突然说自己是语言模型了？
   ...
   原因1：同上一问
   原因2：这是一个概率模型，可以简单的理解为有的ChatGPT好驯服，有的则固执地认为自己是语言模型(可能由于某次他在对话中提到这些内容，但你没有让她忘记或重新生成)，你可以多次强调她是一个猫娘，如果无效的话，reset thread然后从头开始
  
- > 为什么我的或ChatGPT的文字被标黄？

    你可能发了或让ChatGPT说出了不符合服务条款的内容（~~不可以涩涩~~），有可能被封号（~~我到现在也没见到谁涩涩被封号了~~）
    你也可以使用开头的工具禁用审查，以在后续对话避免
